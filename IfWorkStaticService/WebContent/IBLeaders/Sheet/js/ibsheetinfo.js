...
function IBS_SaveNamePrefix() {
    var param = arguments;
    if (param.length < 3) {
        IBS_ShowErrMsg("최하 3개의 인자가 필요합니다.");
        return;
    }

    var attrId = param[1]+"s_SAVENAME";

	$("#"+attrId).remove();
	$("<input></input>",{id:attrId,name:attrId,type:"hidden"}).appendTo(param[0]);

    if (param.length == 3) {
        param[0][attrId].value = IBS_ConcatSaveName(param[2],param[1]);
    } else {
        param[0][attrId].value = "";
        for (var i = 2; i < param.length; i++) {
            param[0][attrId].value += IBS_ConcatSaveName(param[i],param[1]) + "@@";
        }
        var tempStr = param[0][attrId].value;
        param[0][attrId].value = tempStr.substring(0, tempStr.length - 2);
    }
}


function IBS_ConcatSaveName(sheet,prefix) {
