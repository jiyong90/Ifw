								...
								dayWork.plans.map(function(plan){
									var day = moment(plan.key).format('YYYY-MM-DD');
		  		    				...
		  		    				if(plan.valueMap.hasOwnProperty("taaCd") && plan.valueMap.taaCd!='') {
		  		    					...
		  		    					if(plan.valueMap.hasOwnProperty("timeTypeCd") && (plan.valueMap.timeTypeCd=='FIXOT' || plan.valueMap.timeTypeCd=='NIGHT' || plan.valueMap.timeTypeCd=='OTA')) //고정ot, 조출, 야간은 색 다르게 표기
		  		    						timeTypeClass = ' later';
