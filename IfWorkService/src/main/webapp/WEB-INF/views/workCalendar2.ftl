				...
				getWorkDayInfo : function(ymd){ //해당일의 근무 정보
  	         	...
  	         	changeWorkRange : function(){ //근무기간 변경에 따라 background 변경
  	         		...
  	         		if($this.applInfo.useSymd!=null && $this.applInfo.useSymd!='' && $this.applInfo.workRange!=null && $this.applInfo.workRange!='') {
	         			...
	         			console.log($this.selectedFlexitime.workTypeCd);
	         			var classNames = [];
	   		    		...
	   		    		Util.ajax({
							...
							success: function(data) {
								...
								if(data!=null && data.status=='OK') {
									$this.applInfo.flexibleApplId = data.flexibleAppl.flexibleApplId;
									$this.applInfo.applId = data.flexibleAppl.applId;
								}
							},
							error: function(e) {
								console.log(e);
