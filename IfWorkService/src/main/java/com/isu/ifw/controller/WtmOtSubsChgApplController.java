...
@RestController
@RequestMapping(value="/otSubsChgAppl")
public class WtmOtSubsChgApplController {
	...
	WtmApplService otSubsChgApplService;
	...
	public @ResponseBody Map<String, Object> getOtAppl(@RequestParam Long applId
		...
		return otSubsChgApplService.getAppl(tenantId, enterCd, sabun, applId, userId);
	}

	@RequestMapping(value="/request", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody ReturnParam requestChgSubsAppl(@RequestBody Map<String, Object> paramMap
													    , HttpServletRequest request) {
		...
		try {
			...
			if(rp!=null && rp.getStatus()!=null && "OK".equals(rp.getStatus())) {
				otSubsChgApplService.request(tenantId, enterCd, applId, workTypeCd, paramMap, sabun, userId);
			}
