...
import com.isu.ifw.service.WtmTaaCodeService;
import com.isu.ifw.util.WtmUtil;
import com.isu.option.vo.ReturnParam;

@RestController
@RequestMapping(value="/taaCode")
public class WtmTaaCodeController {
	
	@Autowired
	WtmTaaCodeService taaCodeService;

	private final Logger logger = LoggerFactory.getLogger("ifwDBLog");
	
	@RequestMapping(value="/list", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody ReturnParam getTaaCodeList(HttpServletRequest request, @RequestParam Map<String, Object> paramMap ) throws Exception {
		...
		try {		
			taaCodeList = taaCodeService.getTaaCodeList(tenantId, enterCd, paramMap);
			
			rp.put("DATA", taaCodeList);
		} catch(Exception e) {
