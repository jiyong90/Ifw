...
public class IfwLoginController {
	...
	public void loginForRecruitManagement(@PathVariable String tsId, HttpServletRequest request,
		...
		try {
			...
			if(tm == null) {
				response.sendRedirect(request.getContextPath()+"/"+tsId+"/info?status=100");
			}
...
//			}
	...
	public void loginForSso(@PathVariable String tsId, HttpServletRequest request,
		...
		if(userData == null) {
			try {
				response.sendRedirect(request.getContextPath()+"/"+tsId+"/info?status=120");
				return;
			} catch(Exception e) {
				e.printStackTrace();
			}
		...
		try {
			...
			if(emp == null) {
				response.sendRedirect(request.getContextPath()+"/"+tsId+"/info?status=130");
				return;
			}
			...
			e.printStackTrace();
			try {
				response.sendRedirect(request.getContextPath()+"/"+tsId+"/info");
			} catch (IOException e1) {
				e1.printStackTrace();
			}
		} finally {
