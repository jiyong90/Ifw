...
public class IfwLoginController {
	...
	public void loginForRecruitManagement(@PathVariable String tsId, HttpServletRequest request,
		...
		try {
			...
			if(tm == null) {
				response.sendRedirect(request.getContextPath()+"/info/"+tsId+"/100");
			}
...
//			}
	...
	public void loginForSso(@PathVariable String tsId, HttpServletRequest request,
		...
		if(userData == null) {
			try {
				response.sendRedirect(request.getContextPath()+"/info/"+tsId+"/120");
				return;
			} catch(Exception e) {
				e.printStackTrace();
			}
		...
		try {
			...
			if(emp == null) {
				response.sendRedirect(request.getContextPath()+"/info/"+tsId+"/130");
				return;
			}
