...
public interface WtmEmpHisRepository extends JpaRepository<WtmEmpHis, Long> {
	...
	@Query(value="SELECT * FROM WTM_EMP_HIS WHERE TENANT_ID = :tenantId AND ENTER_CD = :enterCd AND (SABUN like %:searchKeyword% OR EMP_NM like %:searchKeyword%) AND IF(:sYmd='',date_format(now(),'%Y%m%d'),:sYmd) BETWEEN SYMD AND EYMD ", nativeQuery = true)
	...
	@Query(value="SELECT * FROM WTM_EMP_HIS WHERE TENANT_ID = :tenantId AND ENTER_CD = :enterCd AND SABUN = :sabun date_format(now(),'%Y%m%d') BETWEEN SYMD AND EYMD ", nativeQuery = true)
